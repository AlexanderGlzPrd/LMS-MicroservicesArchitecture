...
- **El sellado no persiste ningún snapshot de `LessonIds`**: sella `Status` y `CompletedAt`, y nada
  más (ver [ADR-T27](../adr/ADR-T27-sin-instantanea-del-conjunto-de-lecciones.md)). El conjunto publicado pertenece a
...
- El nombre visible se obtiene siempre a través del puerto `IStudentDirectory`. Su fuente definitiva
  es Keycloak Admin API vía ACL ([ADR-T15](../adr/ADR-T15-seguridad-keycloak.md)); hasta ese
  incremento lo satisface un adaptador **provisional**
  ([ADR-T26](../adr/ADR-T26-directorio-provisional-de-estudiantes.md)).
