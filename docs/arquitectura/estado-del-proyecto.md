...
- **Fecha:** 2026-08-17
- **Resumen:** cuatro servicios de dominio ejecutables sobre PostgreSQL y RabbitMQ, más un BFF de
  composición. El proceso completo `matricular → progresar → finalizar → certificar` funciona
  extremo a extremo.
...
| API REST de los cuatro servicios | rutas, verbos, códigos, DTO y validaciones, incluida la verificación pública de certificados | T06, T24 |
...
| Logging | logs estructurados en JSON en los cinco procesos; falta la correlación por `TraceId` entre HTTP y mensajería | T16 |

## Pendiente

| Área | Notas | ADR |
|---|---|---|
| Saga de compra de acceso | `paid-enrollment` y `payment-provider-sim`, con 17 estados y compensaciones —anulación antes de captura, reembolso después—. Está diseñada, no implementada | T13, T23 |
