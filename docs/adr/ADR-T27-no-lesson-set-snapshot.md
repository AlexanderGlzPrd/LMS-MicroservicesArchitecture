...
## Contexto
[T12](./ADR-T12-current-lesson-set.md) decidió que Learning obtiene el **conjunto actual de
...
`docs/diagrams/learning-certification-sequence.md` representaba lo mismo, con el paso
...
- `bounded-contexts.md` §4 enumera el estado del agregado —`StudentId`, `CourseId`, `LessonIds`
...
- La spec que implementó Learning se aprobó y se implementó **sin** ese snapshot: `CourseProgress`
  sella `Status` y `CompletedAt`, y no persiste el conjunto observado.
- `communication-matrix.md` §3 fija la información mínima de `CursoFinalizado` —`StudentId`,
  `CourseId`, fecha— y `bounded-contexts.md` §5 excluye el detalle de lecciones del Certificado.
...
el agregado sin snapshot que describen `bounded-contexts.md` §4 y el código aprobado? Y si es el
...
- **Dejar la discrepancia viva y anotarla en cada spec que la tropiece**: la convierte en folclore y
...
[T12](./ADR-T12-current-lesson-set.md) según la cual el conjunto se obtiene también «para congelar el
...
5. **El total de lecciones del modelo de lectura es una observación derivada, no estado del
   ...
   [T10](./ADR-T10-cqrs-learning.md)— **potencialmente aproximado**. Puede quedar desactualizado tras
...
7. **El estado vigente lo definen `bounded-contexts.md` §4, la spec aprobada que implementó Learning
   y el modelo implementado.** Cualquier lectura futura del Contexto de T12 debe interpretarse a la
   luz de este ADR.
8. **`docs/diagrams/learning-certification-sequence.md` queda alineado**: el paso de sellado ya no
...
`bounded-contexts.md` §4 es el documento que **enumera el estado del agregado**, y no incluye el
...
- El modelo de escritura queda con exactamente el estado que `bounded-contexts.md` §4 enumera.
...
- El Contexto de T12 conserva una frase que ya no describe el sistema, y solo este ADR lo aclara.
  ...
  [T25](./ADR-T25-provisional-progress-bootstrap-retirement.md) aplicó con
  [T24](./ADR-T24-rest-api-versioning.md).
...
[0002](./0002-edicion-de-cursos-y-finalizacion-como-hecho-historico.md) · [T06](./ADR-T06-communication.md) · [T10](./ADR-T10-cqrs-learning.md) · [T12](./ADR-T12-current-lesson-set.md) · [T20](./ADR-T20-contract-versioning.md) · [T25](./ADR-T25-provisional-progress-bootstrap-retirement.md)
