...
- **Eventos de dominio:** `CursoPublicado`, `ContenidoPublicadoModificado`. **No se publican al
  broker**: no tienen consumidor obligatorio (ver [ADR-T12](../adr/ADR-T12-current-lesson-set.md)).
...
- **El conjunto actual de `LessonIds` pertenece a Authoring** y **no forma parte del estado del
  agregado**: se obtiene **fresco en toda escritura** (ver [ADR-T12](../adr/ADR-T12-current-lesson-set.md)).
...
## 6. Extensión académica (fuera del dominio del LMS)
