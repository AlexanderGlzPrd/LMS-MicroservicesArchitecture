# ADR-0030 — `ProgresoDelCurso` no persiste snapshot de `LessonIds`

## Estado
Aceptado — 2026-08-16

## Contexto
[ADR-0015](./ADR-0015-conjunto-vigente-de-lecciones.md) decidió que Learning obtiene el **conjunto actual de
...
El diagrama de secuencia de aprendizaje y certificación representaba lo mismo, con el paso
...
## Problema
¿Cuál es el estado vigente: el `snapshot` que describe el Contexto de ADR-0015 y dibujaba el diagrama, o
el agregado sin snapshot que describen `contextos-delimitados.md` §4 y el código aprobado? Y si es el
segundo, ¿qué ocurre con la afirmación de ADR-0015?

## Alternativas consideradas
- **Implementar el snapshot para hacer cierto el texto de ADR-0015**: ampliaría el modelo de escritura con
...
- **Reescribir el Contexto de ADR-0015**: un ADR aceptado no se edita para acomodar un caso posterior;
  hacerlo borra el rastro de que la discrepancia existió.
- **Dejar la discrepancia viva y aclararla caso por caso**: tarde o temprano alguien lee el texto de
  ADR-0015 aisladamente y implementa el snapshot.
- **Un ADR que declare el estado vigente y sustituya únicamente esa afirmación**, dejando ADR-0015 intacto
  en todo lo demás.
...
[ADR-0015](./ADR-0015-conjunto-vigente-de-lecciones.md) según la cual el conjunto se obtiene también «para congelar el
snapshot al sellar la Finalización». **Todo el resto de ADR-0015 permanece vigente e íntegro.**

**Alcance exacto:**

1. **Learning sigue obteniendo el conjunto fresco de `LessonIds` en toda escritura**, exactamente
   como ADR-0015 exige, en `MarcarLeccionComoCompletada` y en `ConfirmarFinalizacion`. Los dos usos que
   ADR-0015 enumera y que sí siguen vigentes —validar la pertenencia de una `LessonId` y determinar el
...
5. **El total de lecciones del modelo de lectura es una observación derivada, no estado del
   ...
   [ADR-0013](./ADR-0013-cqrs-en-learning.md)— **potencialmente aproximado**. Puede quedar desactualizado tras
...
6. **Las lecciones completadas de la proyección no son un snapshot del conjunto publicado.** Son
   lo que el agregado ya registra: las lecciones que el estudiante completó. Son dos conjuntos
   distintos y esta decisión existe para no confundirlos.
7. **El estado vigente lo definen `contextos-delimitados.md` §4 y el modelo implementado.**
   Cualquier lectura futura del Contexto de ADR-0015 debe interpretarse a la luz de este ADR.
8. **El diagrama de secuencia queda alineado**: el paso de sellado ya no representa ningún snapshot
   de `LessonIds`.
...
## Justificación
Entre dos documentos que se contradicen prevalece el que tiene autoridad sobre la materia en disputa.
`contextos-delimitados.md` §4 es el que **enumera el estado del agregado**, y no incluye el snapshot;
el Contexto de ADR-0015 es la motivación introductoria de una decisión cuyo objeto real es **cómo se
obtiene el conjunto**, no qué se persiste. Su decisión (consulta síncrona fresca en toda escritura)
no depende de que exista o no un snapshot, y por eso sobrevive intacta.
...
introducir en Learning una copia de un dato ajeno, con la obsolescencia y la reconciliación que ADR-0015
descartó expresamente al rechazar la proyección local.

Tampoco hay consumidor. El Integration Event lleva el mínimo fijado; el Certificado excluye el
detalle de lecciones; el modelo de lectura necesita un número, no un conjunto, y lo obtiene como
observación de la escritura que ya está obligada a consultarlo. El snapshot añadiría estado
permanente sin un solo lector.

Queda registrado como ADR, y no como comentario en un cambio concreto, para que la corrección viva
junto a la decisión corregida.
...
- El Contexto de ADR-0015 conserva una frase que ya no describe el sistema, y solo este ADR lo
  aclara. Es la contrapartida de no reescribir un ADR aceptado, el mismo criterio que
  [ADR-0028](./ADR-0028-retirada-de-la-creacion-implicita-de-progreso.md) aplicó con
  [ADR-0027](./ADR-0027-versionado-de-apis-rest.md).
...
## Riesgos residuales
Que alguien lea el Contexto de ADR-0015 aisladamente y vuelva a implementar el snapshot. Se mitiga con
este ADR, enlazado desde el índice, y con el alineamiento del diagrama, que era la otra fuente que lo
representaba.

## Decisiones relacionadas
[ADR-0002](./ADR-0002-edicion-de-cursos-y-finalizacion-inmutable.md) · [ADR-0009](./ADR-0009-comunicacion-sincrona-y-asincrona.md) · [ADR-0013](./ADR-0013-cqrs-en-learning.md) · [ADR-0015](./ADR-0015-conjunto-vigente-de-lecciones.md) · [ADR-0023](./ADR-0023-contratos-de-mensajes-y-versionado.md) · [ADR-0028](./ADR-0028-retirada-de-la-creacion-implicita-de-progreso.md)
