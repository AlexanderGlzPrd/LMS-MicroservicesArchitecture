...
{
    private static readonly ActivitySource ActivitySource = new("learning");
    ...
    {
        if (row.MessageType != OutboxContractMapper.CourseCompletedType)
        {
            throw new UnsupportedOutboxMessageTypeException(row.Id, row.MessageType);
        }

        CourseCompleted? contract;

        try
        {
            contract = JsonSerializer.Deserialize<CourseCompleted>(
